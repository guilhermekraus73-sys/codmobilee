...
const CheckoutForm = () => {
  ...
  useEffect(() => {
    if (stripe && elements) {
      console.log('[Checkout1] Stripe and Elements loaded successfully');
      setStripeReady(true);
    }
  }, [stripe, elements]);
    ...
    pr.on('paymentmethod', async (event) => {
      try {
        ...
        const utmData = {
          ...
          utm_content: utmifyParams.utm_content || baseUtm.utm_content || '',
          utm_term: utmifyParams.utm_term || baseUtm.utm_term || '',
    ...
    const detectCountry = async () => {
      try {
        ...
        if (data.country_code) {
          ...
          console.log('[Checkout1] Country detected:', data.country_code);
        }
      } catch (error) {
        console.log('[Checkout1] Could not detect country, using default');
      }
    };
    detectCountry();
  }, []);
                ...
                <div className="bg-white border-2 border-gray-300 rounded-lg p-3 h-12 flex items-center focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500 transition-all">
                  ...
                  {cardBrand && (
                    <div className="ml-2 flex-shrink-0">
                      ...
                      {cardBrand === 'jcb' && (
                        <img src="https://js.stripe.com/v3/fingerprinted/img/jcb-271fd06e6e7a2c52692c427fab12c24e.svg" alt="JCB" className="h-6" />
                      )}
                      {cardBrand === 'unionpay' && (
                      ...
                      {!['visa', 'mastercard', 'amex', 'discover', 'diners', 'jcb', 'unionpay'].includes(cardBrand) && (
